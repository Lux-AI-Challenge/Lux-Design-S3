...
class LuxAIS3Env(environment.Environment):
    ...
    def compute_unit_counts_map(self, state: EnvState, params: EnvParams):
        ...
        unit_counts_map = jnp.zeros(
            (params.num_teams, params.map_width, params.map_height), dtype=jnp.int32
        )
    ...
    def compute_sensor_masks(self, state, params: EnvParams):
        ...
        def update_vision_power_map(unit_pos, vision_power_map):
            ...
            for i in range(max_sensor_range + 1):
                val = jnp.where(i > max_sensor_range - params.unit_sensor_range, i + 1 - params.unit_sensor_range, 0)
                update = update.at[
        ...
        vision_power_map = (
            vision_power_map
            - (state.map_features.tile_type == NEBULA_TILE)
    ...
    def step_env(
        ...
        def sap_unit(
            ...
            for t in range(params.num_teams):
                other_team_ids = jnp.array(
                    [t2 for t2 in range(params.num_teams) if t2 != t]
                )
                ...
                team_unit_sapped = (
                    ...
                    & (team_sapped_positions[:, 0] < params.map_width)
                    & (team_sapped_positions[:, 1] < params.map_height)
                )
                    ...
                    energy=all_units.energy.at[other_team_ids].set(
                        jnp.where(
                            ...
                            - jnp.array(
                                ...
                                * other_units_adjacent_sapped_count[:, :, None],
                                dtype=jnp.int16,
                            ),
                            all_units.energy[other_team_ids],
                        )
        ...
        unit_aggregate_energy_void_map = jnp.zeros(
            shape=(params.num_teams, params.map_width, params.map_height),
            dtype=jnp.float32,
        )
        ...
        for t in range(params.num_teams):
            ...
            def scan_body(carry, x):
                ...
                agg_energy_map = agg_energy_map.at[
                    unit_position[0], unit_position[1]
                ].add(unit_energy[0] * unit_mask)
                for deltas in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                    new_pos = unit_position + jnp.array(deltas)
                    in_map = (
                        (new_pos[0] >= 0)
                        & (new_pos[0] < params.map_width)
                        & (new_pos[1] >= 0)
                        & (new_pos[1] < params.map_height)
                    )
                    agg_energy_void_map = agg_energy_void_map.at[
                        new_pos[0], new_pos[1]
                    ].add(unit_energy[0] * unit_mask * in_map)
        ...
        for t in range(params.num_teams):
            ...
            team_unit_energy = state.units.energy[t] - jnp.floor(
                jax.vmap(
                    ...
                    * oppposition_energy_void_map[unit_position[0], unit_position[1]]
                    / unit_counts_map[t][unit_position[0], unit_position[1]]
        ...
        def update_unit_energy(unit: UnitState, mask):
            ...
            energy_gain = (
                ...
                - (state.map_features.tile_type[x, y] == NEBULA_TILE)
        ...
        energy_node_deltas = jnp.round(
            jax.random.uniform(
                key=key,
                shape=(params.max_energy_nodes // 2, 2),
        ...
        new_energy_nodes = jnp.clip(
            ...
            min=jnp.array([0, 0]),
            max=jnp.array([params.map_width, params.map_height]),
        )
        ...
        winner_by_energy = jnp.sum(
            state.units.energy[..., 0] * state.units_mask, axis=1
        )
