...
class LuxAIS3Env(environment.Environment):
    ...
    def render(self, state: EnvState, params: EnvParams):
        ...
        for x in range(params.map_width):
            for y in range(params.map_height):
                ...
                if tile_type == 1:
                    color = (166, 177, 225)  # Light blue (a6b1e1) for tile type 1
                else:
                    color = (255, 255, 255)  # White for other tile types
                pygame.draw.rect(self.screen, color, rect)  # Draw filled squares
        ...
        for i in range(params.max_energy_nodes):
            if state.energy_nodes_mask[i]:
                ...
                pygame.draw.circle(self.screen, (0, 255, 0), (int(center_x), int(center_y)), radius)
        # Draw relic nodes
        for i in range(params.max_relic_nodes):
            if state.relic_nodes_mask[i]:
                ...
                pygame.draw.rect(self.screen, (173, 151, 32), rect)  # Light blue color
        ...
        for team in range(2):
            for i in range(params.max_units):
                if state.units_mask[team, i]:
                    ...
                    color = (255, 0, 0) if team == 0 else (0, 0, 255)  # Red for team 0, Blue for team 1
    ...
    def action_space(self, params: EnvParams):
        ...
        return spaces.Dict(
            dict(team_0=MultiDiscrete(size), team_1=MultiDiscrete(size))
        )
